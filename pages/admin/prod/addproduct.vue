...
function handleTaggSelection(event) {
  
  let tgid = taggid.value;
  
    const selectedOptions = Array.from(event.target.selectedOptions).map(option => option.value);

   taggid.value = taggid.value
   .concat(selectedOptions.filter(value => !taggid.value.includes(value)));
 

  console.log("Selected Tag IDs:", taggid.value);
}

function handleTaggSelection2(event) {
  const selectedOptions = Array.from(event.target.selectedOptions).map(option => option.value);
  
  // Remove unselected options
  //taggid.value = taggid.value.filter(value => selectedOptions.includes(value));

  // Add new selected options
  selectedOptions.forEach(option => {
    if (!taggid.value.includes(option)) {
      taggid.value.push(option);
    }
  });
}
					...
					<div class="fullwidth90 bg-white p-8 shadow-md rounded-lg">
						...
						-->  
						  ...
						  <Multiselect
							...
							/>
						   catid  {{ catid }}
						   <span v-if="selectedCat">{{ selectedCat.id }} </span>
						   <span v-else> Nothing cat </span>
						 
						   <span v-if="selectedCat?.id"> Selected Category ID: {{ selectedCat.id }}</span>
						...
						/>
						   subid  {{ subid }}
						   <span v-if="selectedSubCat">{{ selectedSubCat.id }} </span>
						   <span v-else> Nothin sub </span>
						 
						   <span v-if="selectedSubCat?.id"> Selected Category ID: {{ selectedSubCat.id }}</span>
						   <span v-else> No subcategory selected</span>  
						</div>
