...
const fetchSub = async () => {
  try {
    ...
    const response = await $api.get(`${apiUrl}/admin/subcat/detail/${id}`)
	id1.value = id;
...
<div  class="bg-gray-100 font-family-karla flex">
    ...
    <div class="relative w-full flex flex-col h-screen overflow-y-hidden">
        ...
        <header class="w-full items-center bg-white py-2 px-6 hidden sm:flex">
            <div class="w-1/2"></div>
            <div  class="relative w-1/2 flex justify-end">SubCategory Details </div>
        </header>
            ...
            <main class="w-full flex-grow p-6">
				...
				<div class="flex items-center justify-center bg-gray-100">
					<div class="fullwidth90 bg-white p-8 shadow-md rounded-lg">
						<h1 class="text-2xl font-bold mb-4">SubCategory Details</h1>
						<p class="mb-4">
						...
						<div v-if="subcat">	
							<h1> {{ subcat.name }}</h1>
							<h2> {{ subcat.cat.name }}</h2>
							<p>ID: {{ id }}</p>
							<p v-if="subcat.img">
							  <img :src="`${config.public.BaseUrl}/images/subcat/${subcat.img}`"
							  :alt="subcat.name" class="mt-2 mb-2 maxwidth96height400"/>
							</p> 
							<p v-else>
